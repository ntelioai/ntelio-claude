...
var ScriptrMCPServer = class {
  constructor() {
    this.server = new Server(
      {
        name: "scriptr-mcp-server",
        version: "1.0.0"
      },
      {
        capabilities: {
  ...
  setupToolHandlers() {
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      return {
        tools: [
          {
            name: "sync_file",
            description: "Sync a local file to scriptr.io platform",
            inputSchema: {
              type: "object",
              properties: {
                file_path: {
                  type: "string",
                  description: "Path to the file to sync (relative or absolute)"
                },
                instance_url: {
                  type: "string",
                  description: "Scriptr.io instance URL (e.g., 'myinstance.scriptrapps.io')"
                },
                access_token: {
                  type: "string",
                  description: "Scriptr.io API access token"
                },
                remote_path: {
                  type: "string",
                  description: "Optional: Remote path on scriptr.io (defaults to filename)"
                }
              },
              required: ["file_path", "instance_url", "access_token"]
            }
          },
          {
            name: "delete_file",
            description: "Delete a file from scriptr.io platform",
            inputSchema: {
              type: "object",
              properties: {
                file_path: {
                  type: "string",
                  description: "Local file path (used to determine remote path, file doesn't need to exist)"
                },
                instance_url: {
                  type: "string",
                  description: "Scriptr.io instance URL (e.g., 'myinstance.scriptrapps.io')"
          ...
          {
            name: "test_api",
            description: "Test an API endpoint on scriptr.io",
            inputSchema: {
              type: "object",
              properties: {
                endpoint: {
                  type: "string",
                  description: "API endpoint path (e.g., 'myApi' or 'folder/myApi')"
                },
                instance_url: {
                  type: "string",
                  description: "Scriptr.io instance URL (e.g., 'myinstance.scriptrapps.io')"
              ...
              required: ["endpoint", "instance_url", "access_token"]
            }
          }
        ]
      };
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      switch (request.params.name) {
        case "sync_file":
          return await this.syncFile(request.params.arguments);
        case "delete_file":
          return await this.deleteFile(request.params.arguments);
        case "test_api":
          return await this.testApi(request.params.arguments);
        default:
  ...
  async syncFile(args) {
    try {
      const { file_path, instance_url, access_token, remote_path } = args;
      if (!fs.existsSync(file_path)) {
        throw new Error(`File '${file_path}' does not exist`);
      }
      const fileContent = fs.readFileSync(file_path, "utf8");
      const finalRemotePath = remote_path || path.basename(file_path);
      const fileExt = path.extname(file_path).slice(1);
      let contentType = null;
      let acl = null;
      const metadataPath = path.join(path.dirname(file_path), `.${path.basename(file_path)}.metadata`);
      console.error(`DEBUG: Looking for metadata at: ${metadataPath}`);
      if (fs.existsSync(metadataPath)) {
        console.error(`DEBUG: Metadata file found!`);
        try {
          const metadataContent = fs.readFileSync(metadataPath, "utf8");
          console.error(`DEBUG: Metadata content: ${metadataContent}`);
          const metadata = JSON.parse(metadataContent);
          if (metadata.contentType) {
            contentType = metadata.contentType;
            console.error(`DEBUG: Using contentType from metadata: ${contentType}`);
          }
          if (metadata.acl) {
            acl = metadata.acl;
            console.error(`DEBUG: Using ACL from metadata: ${JSON.stringify(acl)}`);
          }
        } catch (metadataError) {
          console.error(`Warning: Could not parse metadata file ${metadataPath}: ${metadataError.message}`);
        }
      } else {
        console.error(`DEBUG: No metadata file found at ${metadataPath}`);
      }
      if (!contentType) {
        switch (fileExt) {
          case "js":
      ...
      if (!acl) {
        const isBrowserResource = contentType !== "application/vnd.scriptr-javascript";
        if (isBrowserResource) {
          acl = { read: "anonymous", write: "nobody", execute: "nobody" };
        } else {
          acl = { read: "nobody", write: "nobody", execute: "authenticated" };
        }
      }
      console.error(`DEBUG: Final ACL being sent - read: ${acl.read}, write: ${acl.write}, execute: ${acl.execute}`);
      console.error(`DEBUG: Final contentType being sent: ${contentType}`);
      const formData = querystring.stringify({
        scriptName: finalRemotePath,
        script: fileContent,
        contentType,
        aclRead: acl.read || "nobody",
        aclWrite: acl.write || "nobody",
        aclExecute: acl.execute || "authenticated"
      });
      const options = {
        hostname: instance_url,
        port: 443,
        path: "/vscodePlugin/scripts",
        method: "POST",
        headers: {
      ...
      if (response.statusCode !== 200) {
        if (response.body.includes('"errorDetail"')) {
          const errorMatch = response.body.match(/"errorDetail":"([^"]+)"/);
          const errorMsg = errorMatch ? errorMatch[1] : "Unknown error";
          throw new Error(`API error (HTTP ${response.statusCode}): ${errorMsg}`);
        } else {
          throw new Error(`HTTP ${response.statusCode}: ${response.body.trim() || "No response body"}`);
        }
      }
      if (response.body.includes('"status":"error"')) {
        const errorMatch = response.body.match(/"errorDetail":"([^"]+)"/);
        const errorMsg = errorMatch ? errorMatch[1] : "Unknown error";
      ...
      return {
        content: [
          {
            type: "text",
            text: `\u2713 Successfully synced ${file_path} to scriptr.io as ${finalRemotePath}`
          }
        ]
      };
    } catch (error) {
  ...
  async deleteFile(args) {
    try {
      const { file_path, instance_url, access_token, remote_path } = args;
      const finalRemotePath = remote_path || path.basename(file_path);
      const encodedScriptName = encodeURIComponent(finalRemotePath);
      const curlCmd = `curl -s -w "%{http_code}" -X DELETE \\
        -H "Authorization: bearer ${access_token}" \\
        "https://${instance_url}/vscodePlugin/scripts?scriptName=${encodedScriptName}"`;
      const { stdout, stderr } = await execAsync(curlCmd);
      const httpStatusMatch = stdout.match(/(\d{3})$/);
      const httpStatus = httpStatusMatch ? httpStatusMatch[1] : null;
      if (httpStatus !== "200") {
        const responseBody = stdout.replace(/\d{3}$/, "");
        if (responseBody.includes('"errorDetail"')) {
          const errorMatch = responseBody.match(/"errorDetail":"([^"]+)"/);
          const errorMsg = errorMatch ? errorMatch[1] : "Unknown error";
          throw new Error(`API error (HTTP ${httpStatus}): ${errorMsg}`);
        } else {
          throw new Error(`HTTP ${httpStatus}: ${responseBody.trim() || "No response body"}`);
        }
      }
      return {
        content: [
          {
            type: "text",
            text: `\u2713 Successfully deleted ${finalRemotePath} from scriptr.io`
          }
        ]
      };
    } catch (error) {
  ...
  async testApi(args) {
    try {
      const { endpoint, instance_url, access_token, method = "GET", params = {} } = args;
      const apiUrl = `https://${instance_url}/${endpoint}`;
      let curlCmd = `curl -s -X ${method}`;
      curlCmd += ` -H "Authorization: bearer ${access_token}"`;
      if (method === "POST" || method === "PUT") {
        curlCmd += ` -H "Content-Type: application/json"`;
        if (Object.keys(params).length > 0) {
          curlCmd += ` -d '${JSON.stringify(params)}'`;
        }
        curlCmd += ` "${apiUrl}"`;
      } else if (method === "GET" || method === "DELETE") {
        if (Object.keys(params).length > 0) {
          const queryString = new URLSearchParams(params).toString();
          curlCmd += ` "${apiUrl}?${queryString}"`;
        } else {
          curlCmd += ` "${apiUrl}"`;
        }
      } else {
      ...
      return {
        content: [
          {
            type: "text",
            text: `\u2713 API test completed

Endpoint: ${method} ${apiUrl}
...
${stderr}` : ""}`
  ...
  async run() {
    ...
    console.error("Scriptr MCP server running on stdio");
  }
};
var server = new ScriptrMCPServer();
